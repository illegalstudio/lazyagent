...
func main() {
	...
	agentMode := flag.String("agent", "all", "Which agent sessions to show: claude, pi, opencode, cursor, amp, all (default: all)")
...
`)
	...
	if *demoMode {
		...
		switch *agentMode {
		case "claude", "pi", "opencode", "cursor", "amp", "all":
			provider = core.BuildProvider(*agentMode, cfg)
		default:
			fmt.Fprintf(os.Stderr, "Error: unknown --agent value %q (use claude, pi, opencode, cursor, amp, or all)\n", *agentMode)
			os.Exit(1)
		}
	}
